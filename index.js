...
function isBlack({r, g, b}) {
    return r < 75 && r > 45 && g < 70 && g > 45 && b > 80 && b < 115
}
...
function random(min, max) {
    return parseInt(Math.random() * max + max - min)
}
...
async function getPosition() {
    try {
        ...
        let {r: r5, g: g5, b: b5} = getRGB(pixels, width / 2, height - 10)
    ...
    setTimeout(() => {
        getPosition().then((res) => {
            if (res) {
                ...
                // 随机按压,避开微信的防外挂检测
                jump(random(400, 800), random(600, 1200), parseInt(press_time))
                // 随机停顿,也是为了避开微信的防外挂检测
                console.log('等待中...')
